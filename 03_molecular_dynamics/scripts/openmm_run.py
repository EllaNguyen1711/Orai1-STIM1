...
# Define system used for simulation
sys_name = os.path.basename(os.path.dirname(args.topfile)) 
...
if inputs.rest == 'yes':         system = restraints(system, crd, inputs, sys_name)
...
if inputs.mini_nstep > 0:
    ...
    # === write checkpoint after minimization ===
    if args.ochk:
        print("\nWriting minimization checkpoint:", args.ochk)
        simulation.saveCheckpoint(args.ochk)
...
if inputs.nstep > 0:
    ...
    if inputs.nstdcd > 0:
        ...
        if not args.ochk: args.ochk = 'output.chk' # Prevention of sudden corruption.
...
elapsed = end - start
print("Simulation done within :", elapsed)
