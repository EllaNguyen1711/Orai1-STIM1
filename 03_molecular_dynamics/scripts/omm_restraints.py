...
CWD = os.path.abspath(os.path.join(SCRIPT_DIR, '..')) # base directory where all sub directories stored!
restraints_dir = os.path.join(CWD, 'restraints')

def restraints(system, crd, inputs, sys_name):
    ...
    if inputs.fc_bb > 0 or inputs.fc_sc > 0:
        ...
        for line in open(f'{restraints_dir}/{sys_name}/prot_pos.txt', 'r'):
            ...
            if state == 'BB' and inputs.fc_bb > 0: # For backbone
                fc_ppos = inputs.fc_bb
                posresPROT.addParticle(atom1, [fc_ppos, xpos, ypos, zpos])
            if state == 'SC' and inputs.fc_sc > 0: # For side-chain
    ...
    if inputs.fc_mpos > 0:
        ...
        for line in open(f'{restraints_dir}/{sys_name}/lipid_pos.txt', 'r'):
            segments = line.strip().split()
            atom1 = int(segments[0])
            xpos  = crd.positions[atom1].value_in_unit(nanometers)[0]
            ypos  = crd.positions[atom1].value_in_unit(nanometers)[1]
    ...
    if inputs.fc_hmmm > 0:
        ...
        for i, line in enumerate(open(f'{restraints_dir}/{sys_name}/hmmm_pos.txt', 'r')):
            segments = line.strip().split()
    ...
    if inputs.fc_dcle > 0:
        ...
        # add groups and bonds
        for i, line in enumerate(open(f'{restraints_dir}/{sys_name}/dcle_pos.txt', 'r')):
    ...
    if inputs.fc_ldih > 0:
        ...
        for line in open(f'{restraints_dir}/{sys_name}/dihe.txt', 'r'):
            segments = line.strip().split()
            atom1  = int(segments[0])
    ...
    if inputs.fc_cdih > 0:
        ...
        if os.path.isfile(f'{restraints_dir}/{sys_name}/carbohydrate_restraint.dat'):
            for line in open(f'{restraints_dir}/{sys_name}/carbohydrate_restraint.dat', 'r'):
                segments = line.strip().split()
                atom1  = int(segments[0])
                atom2  = int(segments[1])
                atom3  = int(segments[2])
        ...
        if os.path.isfile(f'{restraints_dir}/{sys_name}/detergent_carbohydrate_restraint.dat'):
            for line in open(f'{restraints_dir}/{sys_name}/detergent_carbohydrate_restraint.dat', 'r'):
                segments = line.strip().split()
                atom1  = int(segments[0])
                atom2  = int(segments[1])
                atom3  = int(segments[2])
